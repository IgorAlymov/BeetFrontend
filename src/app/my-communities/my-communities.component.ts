...
export class MyCommunitiesComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.dataService.getMyCommunities(data.socialUserId).subscribe((data) => this.getAvatarCommunity(data))
    );
  }
    ...
    this.dataService.deleteCommunity(idG).subscribe(
      p => this.allCommunity.forEach(element => {
        if(element.groupId==idG){
          ...
          element.subscribers=this.dataService.getCommunitySubscribers(element.groupId).subscribe((data:any) => element.subscribers=data)
          for(let i = 0; i < this.allCommunity.length; i++) {
            if(this.allCommunity[i].groupId==idG)
            this.allCommunity.splice(i,1);
          }
        }
      })
    );
